...
public interface ProductMapper {
     ...
     * 查询所有上架商品，按排序序号排序
     */
    @Select("SELECT id, name, description, price, original_price AS originalPrice, sales_count AS salesCount, stock_count AS stockCount, " +
     ...
     * 插入商品
     */
    @Insert("INSERT INTO product (name, description, price, original_price, sales_count, stock_count, category, tags, status, sort_order, create_time, update_time) " +
    ...
    @Update("UPDATE product SET name = #{name}, description = #{description}, price = #{price}, original_price = #{originalPrice}, " +
            "sales_count = #{salesCount}, stock_count = #{stockCount}, category = #{category}, tags = #{tags}, " +
